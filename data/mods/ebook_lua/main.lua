...
mod.ebook_scan = function ( user, device )
    ...
    if have_no_book then
        ...
        local base_text = string.format(locale.gettext("%d books are found. Do you want to scan them all?\n"), mod.arr_num(found))
    ...
    while true do
        ...
        if sel < 0 then
            ...
            elseif minute > device.charges - 10 then
                ...
                book_in_real:set_var_str("item_label", string.format(locale.gettext("[e-copy] %s"), string.gsub(selected.name, " %(unread%)", ""))) -- no more "X (unread) (unread)"
                ...
                if book_in_real.charges ~= 0 then
                    gdebug.log_info(string.format("%s has %d charge(s). Why does it happen?", selected.type:str(), book_in_real.charges))
...
mod.ebook_return = function ( reader, device )
    ...
    for key, it in pairs(virtual_books) do
        ui_return:add_w_col( key, it:tname(1, false, letter_limit), "NO_DESC", string.format("+%d%%", math.floor(it:get_var_num("ethereal", 0) / 60 / device:ammo_capacity(false) * 100)))
    end
...
mod.check_firmware = function (reader, device)
    ...
    ui_firmware:add_w_desc(2, "Try to jailbreak","Attempt to exploit firmware vulnerabilities to lift access restrictions.\nWith enough skill (and the right hacking software), you just might pull it off." ) --"펌웨어의 보안 취약점을 찾아서 기능의 접근 제한을 해제합니다.\n당신의 실력과 해킹 소프트웨어만 있다면 할 수 있을 것 같군요."
    while true do
        local fw = device:get_var_str("tablet_fw", base_fw)
        ui_firmware:text(string.format(locale.gettext("UID: %s\nFirmware: %s\nTrying to fetching data from server... fail\n\nNewest firmware is: %s"), uid, fw, base_fw))
        ...
        if ans_mc_sel <= 0 then
            ...
            while true do
                ...
                elseif ans_mc_menu == 0 then --List
                    ...
                    mod.poppin(string.format(locale.gettext("Book data of %s:\n%s"), that_mc:tname(1,false,0), book_data_mc_text))
                elseif ans_mc_menu == 1 then --Download
                    ...
                    if dl_count == 0 then
                        ...
                        mod.poppin(string.format(locale.gettext("%d books downloaded from %s."), dl_count, that_mc:tname(1,false,0) ))
                    end
                elseif ans_mc_menu == 2 then --Upload
                    ...
                    if ul_count == 0 then
                        ...
                        mod.poppin(string.format(locale.gettext("%d books uploaded on %s.\nNaming the card is recommended."), ul_count, that_mc:tname(1,false,0)))
                ...
                elseif ans_mc_menu == 3 then --Remove all
                    if not that_mc:has_var("book_data") then
                        ...
                        local yn1 = mod.poppyn(string.format(locale.gettext("Do you want to delete all of book data of %s? It's irretrievable!"), that_mc:tname(1,false,0)))
                        if yn1 == "YES" then
...
mod.reset_lib = function (reader, device)
    local yn1 = mod.poppyn(string.format(locale.gettext("Do you want to delete all of book data of %s? It's irretrievable!"), device:tname(1,false,0)))
    if yn1 == "YES" then
...
mod.ebook_info = function (reader, device)
    ...
    while true do
        ...
        ui_info:text(string.format(locale.gettext("UID: %s\nFirmware: %s\nNetwork: Offline (Cloud unreachable)\n\n\nLast Sync: %d days ago"), uid, fw, day))
...
mod.ebook_ui = function (who, item, pos)
    ...
    ui:text(string.format(locale.gettext("Welcome to e-book library!\nThis device currently holds %d book(s).\nNetwork sync unavilable."), uid, var_count))
    ui:add_w_desc(-1, locale.gettext("Scan book(s)"), locale.gettext("Scans all the book you have. It will progress instantly."))
    ui:add_w_desc(-1, locale.gettext("Load book(s)"), locale.gettext("Temporarily prints a physical copy from the device’s library."))
    ui:add_w_desc(-1, locale.gettext("Return book(s)"), locale.gettext("Absorbs the copy back into the device and restores partial power."))
