		...
		for j = 1,const.OMT_MS_SIZE do
			...
			--gapi.get_map():set_furn_at( xyz, FurnId.new("f_fridge"):int_id() )
				if furn_at_tile == furniture_name then
					...
					gapi.add_msg(locale.gettext("You should get the".. tostring(spawn_item).. "item back at this point, but not yet available in LUA, sorry! Feel free to debug spawn the appropriate item."))
					return 0
