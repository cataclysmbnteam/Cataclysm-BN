...
{
    double const percent = get_option<int>( "MAGIC_STAT_SCALING_PERCENT" ) / 100.0;
    if( decrease ) {
...
int spell::damage_as_character( const Character &guy ) const
{
    ...
    if( has_flag( spell_flag::ADD_MELEE_DAM ) ) {
        item  const &weapon = guy.used_weapon();
        int weapon_damage = 0;
        if( !weapon.is_null() ) {
...
int spell::energy_cost( const Character &guy ) const
{
    ...
    if( type->base_energy_cost < type->final_energy_cost ) {
        cost = std::min( type->final_energy_cost,
                         static_cast<int>( std::round( type->base_energy_cost + ( type->energy_increment *
                                           get_level() ) ) ) );
...
bool spell::is_valid_target( const Creature &caster, const tripoint &p ) const
{
    ...
    if( Creature *const cr = g->critter_at<Creature>( p ) ) {
        Attitude const cr_att = cr->attitude_to( caster );
...
{
    bool const is_flat_rate = get_option<bool>( "MANA_REGEN_IS_FLAT" );
    double base_rate;
    if( !is_flat_rate ) {
        // mana should replenish in hours_to_regen hours by default.
        int const hours_to_regen = get_option<int>( "MANA_REGEN_HOURS_RATE" );
        double const full_replenish = to_turns<double>( time_duration::from_hours( hours_to_regen ) );
    ...
    double const natural_regen = std::max( 0.0, base_rate * mut_mul );
