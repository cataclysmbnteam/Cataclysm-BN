...
std::vector<npc *> player_activity::get_assistants( const Character &who, unsigned short max )
{
    ...
    return g->get_npcs_if( [&]( const npc & guy ) {
        ...
        bool const ok = guy.is_npc() && !guy.in_sleep_state() && guy.is_obeying( who ) &&
...
{
    unsigned short const max = type->max_assistants();
    if( max < 1 ) {
        assistants_ = {};
        return;
    }
                               ...
                               bool force_show = false )
{
    ...
    int const percent = static_cast<int>( std::roundf( level * 100.0f ) );
    nc_color const col = percent == 100
                         ? c_white
...
void player_activity::do_turn( player &p )
{
    ...
    active = true;
    on_out_of_scope const _resolve_on_return( [this]() {
        this->resolve_active();
    } );
    ...
    */
    if( !type->special() ) {
        if( type->complex_moves() ) {
            ...
            int const moves_total = speed.total_moves();
