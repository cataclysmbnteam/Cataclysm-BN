...
void show_lua_console_impl()
{
    ...
    ui.on_redraw( [&]( const ui_adaptor & ) {
        ...
        const int separator_y = win_size.y - prompt_size.y - 2;
        ...
        const int start_line = log_scroll_pos;
        const int end_line = std::min( static_cast<int>( log_folded.size() ), start_line + log_size.y );
        for( int log_line = start_line; log_line < end_line; log_line++ ) {
            const folded_log_msg &msg = log_folded[log_line];
            const nc_color col = get_log_level_color( msg.level );
            int x_pos = 1;
    ...
    while( true ) {
        ...
        if( act == "QUIT" ) {
            ...
            const int sz = num_history_entries();
            ...
            const int limit = std::max( 0, static_cast<int>( log_folded.size() ) - log_size.y );
            if( act == "SCROLL_TOP" ) {
