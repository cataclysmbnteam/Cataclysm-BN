...
inline auto UTF8_getch( const std::string &str ) -> uint32_t
{
...
auto cursorx_to_position( const char *line, int cursorx, int *prevpos = nullptr, int maxlen = -1 ) -> int;
auto utf8_width( const char *s, bool ignore_tags = false ) -> int;
auto utf8_width( const std::string &str, bool ignore_tags = false ) -> int;
auto utf8_width( const utf8_wrapper &str, bool ignore_tags = false ) -> int;

auto left_justify( const std::string &str, int width, bool ignore_tags = false ) -> std::string;
auto right_justify( const std::string &str, int width, bool ignore_tags = false ) -> std::string;
auto utf8_justify( const std::string &str, int width, bool ignore_tags = false ) -> std::string;
...
auto utf8_display_split( const std::string & ) -> std::vector<std::string>;
...
class utf8_wrapper
{
    public:
        ...
        auto substr( size_t start, size_t length ) const -> utf8_wrapper;
        auto substr( size_t start ) const -> utf8_wrapper {
        ...
        auto replace_all( const utf8_wrapper &search, const utf8_wrapper &replace ) -> utf8_wrapper &;
        /**
         ...
         */
        auto substr_display( size_t start, size_t length ) const -> utf8_wrapper;
        auto substr_display( size_t start ) const -> utf8_wrapper {
            return substr_display( start, _length - start );
        }

        auto operator=( const std::string &d ) -> utf8_wrapper & {
            *this = utf8_wrapper( d );
            return *this;
        }
        ...
        auto at( size_t start ) const -> uint32_t;
        ...
        auto display_width() const -> size_t {
         ...
         * characters.
         */
        auto shorten( size_t maxlength ) const -> std::string;
    protected:
        std::string _data;
        size_t _length;
        ...
        auto byte_start( size_t bstart, size_t start ) const -> size_t;
        ...
        auto byte_start_display( size_t bstart, size_t start ) const -> size_t;
        // Same as @ref substr, but with a byte index as start
        auto substr_byte( size_t bytestart, size_t length, bool use_display_width ) const -> utf8_wrapper;
