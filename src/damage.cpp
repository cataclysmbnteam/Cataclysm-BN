...
std::map<damage_type, float> load_damage_map( const JsonObject &jo )
{
    ...
    std::optional<float> const init_val = jo.has_float( "all" ) ?
    ...
    std::optional<float> fallback ) {
        if( jo.has_float( name ) ) {
            float const val = jo.get_float( name );
            ret[dt] = val;
        } else if( fallback ) {
            ret[dt] = *fallback;
        }
    ...
    std::optional<float> const phys = jo.has_float( "physical" ) ?
    ...
    std::optional<float> const non_phys = jo.has_float( "non_physical" ) ?
             ...
             bool /*strict*/ )
{
    ...
    if( relative.has_member( name ) ) {
        err = relative;
        JsonObject const jo_relative = err.get_member( name );
        ...
        err = relative;
        JsonObject const jo_proportional = err.get_member( name );
        ...
        JsonObject const jo_inner = jo.get_object( name );
