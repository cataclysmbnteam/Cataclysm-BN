...
{
    double const ret = 10'000'000.0 / moves_total * ( moves_total - moves_left );
...
void aim_activity_actor::finish( player_activity &act, Character &who )
{
    ...
    int const shots_fired = ranged::fire_gun( who, fin_trajectory.back(), gun.qty, *gun, ammo_loc );
    ...
    shared_ptr_fast<Creature> const last_target = who.last_target.lock();
...
void dig_activity_actor::start( player_activity &/*act*/, Character & )
{
    ...
    ter_id const ter_here = here.ter( location );
...
{
    hack_type const type = get_hack_type( act.placement );
    std::string name;
...
void move_items_activity_actor::do_turn( player_activity &act, Character &who )
{
    ...
    while( who.moves > 0 && !target_items.empty() ) {
        safe_reference<item> const target = std::move( target_items.back() );
...
void lockpick_activity_actor::finish( player_activity &act, Character &who )
{
    ...
    bool const perfect = it->has_flag( flag_PERFECT_LOCKPICK );
    bool destroy = false;
...
void throw_activity_actor::do_turn( player_activity &act, Character &who )
{
    ...
    if( it != &who.primary_weapon() ) {
        ...
        int const extra_cost = who.item_handling_cost( *it, true, INVENTORY_HANDLING_PENALTY / 2 );
...
void construction_activity_actor::start( player_activity &/*act*/, Character &/*who*/ )
{
    ...
    int const total_time = std::max( 1, built.adjusted_time() );
    int const left = pc->counter == 0
                     ? total_time
