...
void ebooksave_activity_actor::do_turn( player_activity &act, Character &who )
{
    ...
    if( calendar::once_every( 25 * time_per_page ) ) {
        if( !ereader->ammo_sufficient() ) {
            who.add_msg_if_player( string_format( "The %s ran out of batteries.",
                                                  item::nname( ereader->typeId() ) ) );
            who.cancel_activity();
            return;
        }

        ereader->ammo_consume( ereader->ammo_required(), who.pos() );
    }
...
{
    item book_copy = *book;
    ereader->put_in( book_copy );
    if( who.is_avatar() ) {
        who.add_msg_if_player( m_info, _( "You scan the book into your device." ) );
    }
    act.set_to_null();
