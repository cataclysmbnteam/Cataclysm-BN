                  ...
                  int init_veh_status ): type( type_id )
{
    ...
    if( !type.str().empty() && type.is_valid() ) {
        ...
        for( vehicle_part  const &part : proto.blueprint->parts ) {
...
void vehicle::init_state( int init_veh_fuel, int init_veh_status )
{
    ...
    for( const vpart_reference &vp : get_all_parts() ) {
        ...
        if( veh_status == 0 ) {
            ...
            int const broken = 8 * chance;
            int const unhurt = 20 * chance;
    ...
    if( destroyTires && !wheelcache.empty() ) {
        int tries = 0;
        int const maxtries = wheelcache.size();
...
int vehicle::install_part( point dp, const vpart_id &id, bool force )
{
    ...
    int const ret = install_part( dp, vehicle_part( id, dp, std::move( obj ), this ) );
    return ret;
}
...
{
    item  const &fuel = *item::spawn_temporary( ftype );
...
int vehicle::ground_acceleration( const bool fueled, int at_vel_in_vmi ) const
{
    ...
    int const target_vmiph = std::max( {at_vel_in_vmi, 1000, max_velocity( fueled ) / 4 } );
...
int vehicle::water_acceleration( const bool fueled, int at_vel_in_vmi ) const
{
    ...
    int const target_vmiph = std::max( {at_vel_in_vmi, 1000,
...
int vehicle::damage( int p, int dmg, damage_type type, bool aimed )
{
    ...
    if( armor_part < 0 ) {
        ...
        int const protection = part_info( armor_part ).damage_reduction.type_resist( type );
    ...
    for( const vpart_reference &vp : get_all_parts() ) {
        ...
        if( distance > 1 ) {
            ...
            if( shockpart.location != part_location_structure ||
                ...
                int const shock_absorber = part_with_feature( p, "SHOCK_ABSORBER", true );
                if( shock_absorber >= 0 ) {
...
{
    int i = 0;
    for( vehicle_part  const &part : parts ) {
        if( part.hack_id == id ) {
            return i;
        }
        i++;
