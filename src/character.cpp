                            ...
                            const bodypart_str_id &bp2_id )
{
    ...
    int const diff = static_cast<int>( ( bp2.get_temp_cur() - bp1.get_temp_cur() ) * 0.001 );
...
int Character::swim_speed() const
{
    ...
    float const hand_bonus_mult = ( usable.test( STATIC( bodypart_str_id( "hand_l" ) ) ) ? 0.5f : 0.0f )
                                  +
...
void Character::calc_all_parts_hp( float hp_mod, float hp_adjustment, int str_max )
{
    for( std::pair<const bodypart_str_id, bodypart>  const &part : get_body() ) {
        ...
        float const hp_ratio = static_cast<float>( bp.get_hp_cur() ) / bp.get_hp_max();
        ...
        int const new_cur = std::ceil( static_cast<float>( new_max ) * hp_ratio );
...
ret_val<bool> Character::can_swap( const item &it ) const
{
    if( it.has_flag( flag_POWERARMOR_MOD ) ) {
        ...
        for( auto &elem : worn ) {
            for( std::pair< bodypart_str_id, int > &mod_part : mod_parts ) {
                bodypart_str_id const bpid = mod_part.first;
        ...
        const bodypart_id &bp )
{
    ...
    for( const item *it : items ) {
        ...
        int const coverage = std::max( 0, i.get_coverage( bp ) - penalty );
...
void Character::update_health( int external_modifiers )
{
    ...
    float const health_change = effective_healthy_mod - get_healthy() + external_modifiers;
...
void Character::update_bodytemp( const map &m, const weather_manager &weather )
{
    ...
    // In Celsius / 100
    int const Ctemperature = units::to_millidegree_celsius( player_local_temp ) / 10;
    ...
    for( auto &pr : get_body() ) {
        ...
        int const temp_after = bp_stats.get_temp_cur();
        // PENALTIES
        ...
        **/
        ...
        if( bp == body_part_mouth || bp == body_part_foot_r ||
            ...
            int const wetness_percentage = 100 * bp_stats.get_wetness() /
            ...
            int const Ftemperature = static_cast<int>( units::to_fahrenheit( player_local_temp ) + ( 0.2 *
                                     adjusted_warmth ) );
            // Windchill reduced by your armor
            int const FBwindPower = static_cast<int>(
                                        total_windpower * ( 1 - wind_res_per_bp[ bp ] / 100.0 ) );

            int const intense = get_effect_int( effect_frostbite, bp.id() );
        ...
        if( in_sleep_state() ) {
            int const curr_temperature = bp_stats.get_temp_cur();
                                        ...
                                        float bleed, float bite, float infect, float bandage_power, float disinfectant_power ) const
{
    ...
    for( const bodypart_id &bp : get_all_body_parts( true ) ) {
        // Ugly!
        int const heal_bonus = bp == body_part_head ? head_bonus :
    ...
    for( size_t i = 0; i < parts.size(); i++ ) {
        ...
        std::string msg;
        std::string desc;
        bool const bleeding = has_effect( effect_bleed, bp_str_id );
        bool const bitten = has_effect( effect_bite, bp_str_id );
        bool const infected = has_effect( effect_infected, bp_str_id );
        bool const bandaged = has_effect( effect_bandaged, bp_str_id );
        bool const disinfected = has_effect( effect_disinfected, bp_str_id );
...
float Character::stamina_burn_cost_modifier() const
{
    ...
    float const stamina_modifier = ( static_cast<float>( get_stamina() ) / get_stamina_max() + 1 ) / 2;
                        ...
                        dealt_projectile_attack const *const proj )
{
    ...
    bool const in_skater_vehicle = in_vehicle &&
        ...
        &clothing_map ) const
{
    ...
    for( const std::pair<const bodypart_id, std::vector<const item *>> &on_bp : clothing_map ) {
        ...
        for( const item *it : on_bp.second ) {
            ...
            float const wet_mult = 1.0f - ( max_wet_resistance * wetness_map[bp] );
                             ...
                             const std::function<bool( const item & )> &filter )
{
    ...
    if( qty <= 0 ) {
        ...
        if( is_mounted() && mounted_creature.get()->has_flag( MF_RIDEABLE_MECH ) &&
            ...
            auto mons = mounted_creature.get();
            int const power_drain = std::min( mons->get_battery_item()->ammo_remaining(), qty );
        ...
        int const adv = charges_of( itype_adv_UPS_off, static_cast<int>( std::ceil( qty * 0.5 ) ) );
        if( adv > 0 ) {
            int const adv_odd = x_in_y( qty % 2, 2 );
...
int Character::impact( const int force, const tripoint &p )
{
    ...
    if( critter != this && critter != nullptr ) {
        ...
        int const hard_ground = g->m.ter( p )->is_diggable() ? 0 : 3;
        armor_eff = 0.25f; // Not much
        // Get cut by stuff
...
int Character::item_reload_cost( const item &it, item &ammo, int qty ) const
{
    ...
    int const saved_quantity = ammo.charges;
    ammo.charges = qty;
    ...
    int const cost = ( it.is_gun() ? it.get_reload_time() : it.type->magazine->reload_time ) *
                     ( it.is_gun() ? 1 : qty );
