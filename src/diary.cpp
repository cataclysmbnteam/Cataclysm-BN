...
std::string diary::get_head_text()
{
    ...
    if( !pages.empty() ) {
        ...
        std::string headtext = string_format( _( "Entry: %d/%d, %s, %s" ),
                                              ...
                                              ( opened_page != 0 ) ? string_format( _( std::string( ( 50 - leftside_length ),
                                                      ' ' ) + "%s%s%d minutes since last entry" ),
...
{
    std::ofstream myfile;
    std::string path = lastexport ? PATH_INFO::memorialdir() : g->get_world_base_save_path();
