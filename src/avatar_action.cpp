...
bool avatar_action::move( avatar &you, map &m, const tripoint &d )
{
    ...
    if( monster *const mon_ptr = g->critter_at<monster>( dest_loc, true ) ) {
        ...
        monster_attitude const att = critter.attitude( ( &u ) );
                             ...
                             const std::optional<tripoint> &blind_throw_from_pos )
{
    ...
    int const range = you.throw_range( *loc );
    if( range < 0 ) {
...
void avatar_action::wield( item &loc )
{
    ...
    item *to_wield = &loc;
    item_location_type const location_type = loc.where();
    ...
    if( u.is_worn( loc ) ) {
        ...
        int const item_pos = u.get_item_position( &loc );
        if( item_pos != INT_MIN ) {
...
void avatar_action::reload( item &loc, bool prompt, bool empty )
{
    ...
    item_reload_option const opt = favorite_ammo_or_select( u, *it, empty, prompt );
...
void avatar_action::reload_weapon( bool try_everything )
{
    ...
    if( veh && ( turret = veh->turret_query( u.pos() ) ) && turret.can_reload() ) {
        item_reload_option const opt = character_funcs::select_ammo( u, turret.base(), true );
        if( opt ) {
