...
static bool mx_house_wasp( map &m, const tripoint &loc )
{
    ...
    if( one_in( 5 ) ) {
        m.add_spawn( mon_dermatik, rng( 1, 3 ), tripoint( point( ( SEEX * 2 ) - 1, ( SEEY * 2 ) - 1 ),
                     loc.z ) );
    }
    return true;
}
...
{
    int const size = rng( 5, 8 );
    int const size_squared = size * size;
    int const size_center = rng( 1, 3 );
    int const size_center_squared = size_center * size_center;
