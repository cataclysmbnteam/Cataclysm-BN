...
static bool stream_write_values( std::ostream &os, const char delim, const First &val,
                                 const Rest &... rest )
{
...
static bool stream_write_indexed( std::ostream &os, const char delim, const T &val, Fn &&getter,
...
template<typename T, typename Fn, size_t ... Is>
static bool stream_read_indexed( std::istream &is, const char delim, T &val, Fn &&getter,
                                 std::index_sequence<Is...> )
{
...
struct stream_value_io<T> {
    ...
    bool operator()( std::ostream &os, const char delim, const T &val ) const {
        os << '[';
        detail::stream_write_indexed( os, delim, val, tuple_get{},
    ...
    bool operator()( std::istream &is, const char delim, T &val ) const {
        ...
        detail::stream_read_indexed( is, delim, val, tuple_get{},
                                     std::make_index_sequence<std::tuple_size_v<T>>() );
        is >> c >> std::ws;
        if( c != ']' ) {
            return false;
