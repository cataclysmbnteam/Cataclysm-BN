...
void complete_salvage( Character &who, item &cut, tripoint_abs_ms pos )
{
    ...
    int const entropy_threshold = std::max( 5, 10 - who.get_skill_level( skill_fabrication ) );
    ...
    if( cut.damage() > 0 ) {
        float const component_success_chance = std::min( std::pow( 0.8, cut.damage_level( 4 ) ),
                                               1.0 );
    ...
    for( const auto &salvaged : salvage_results( cut ) ) {
        int const amount = std::floor( salvagable_percent * salvaged.second );
        if( amount > 0 ) {
...
bool prompt_salvage_single( Character &who, item &target )
{
    ...
    for( const auto &component : components ) {
        int const c = std::floor( component.second );
    ...
    iuse_location const loc( target, 0 );
...
bool salvage_all( Character &who )
{
    ...
    if( !targets.empty() ) {
        tripoint_abs_ms const pos_abs( here.getabs( pos ) );
