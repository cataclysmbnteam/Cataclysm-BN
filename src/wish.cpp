...
void debug_menu::wishbionics( Character &c )
{
    ...
    while( true ) {
        ...
        size_t const num_installed = c.get_bionic_collection().size();
    ...
    public:
        ...
        void refresh( uilist *menu ) override {
            ...
            if( entnum >= 0 && static_cast<size_t>( entnum ) < standard_itype_ids.size() ) {
                item  const &tmp = *item::spawn_temporary( standard_itype_ids[entnum], calendar::turn );
...
void debug_menu::wishitem( Character *who, const tripoint &pos )
{
    ...
    for( size_t i = 0; i < opts.size(); i++ ) {
        //TODO!: push up
        item  const &ity = *item::spawn_temporary( opts[i].second, calendar::start_of_cataclysm );
        ...
        if( skill_id >= 0 && skset >= 0 ) {
            ...
            if( ret >= 0 ) {
                ...
                for( size_t skill_id = 0; skill_id < sorted_skills.size(); skill_id++ ) {
                    const Skill &skill = *sorted_skills[skill_id];
                    int const changeto = skmod != 0 ? who->get_skill_level( skill.ident() ) + skmod :
