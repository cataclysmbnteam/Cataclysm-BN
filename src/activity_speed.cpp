...
{
    float const ac_f = skills_factor_custom_formula( who, skill_req );
    ...
    for( const auto &skill : skill_req ) {
        int const who_eff_skill = who.get_skill_level( skill.req ) - skill.threshold;
        float bonus = 0;
        if( who_eff_skill != 0 ) {
        ...
        const activity_req<character_stat> &stat )
{
    ...
    float const f = 1.0f + refine_factor( stat.mod * ( who_stat - stat.threshold ) );
        ...
        const inventory &inv )
{
    ...
    inv.visit_items( [&q, &q_level]( const item * itm ) {
        int const new_q = itm->get_quality( q.req );
    ...
    if( q.req == qual_CUT_FINE ) {
        float const cut_fine_f = ( 2.0f * std::pow( q_level, 3 ) )
...
void activity_speed::calc_tools_factor( Character &who, const q_reqs &quality_reqs )
{
    ...
    float const ac_f = tools_factor_custom_formula( quality_reqs, inv );
    //Any factor above 0 is valid, else - use default calc
    if( ac_f > 0 ) {
        tools = ac_f;
        return;
...
void activity_speed::calc_morale_factor( const Character &who )
{
    ...
    float const ac_morale = morale_factor_custom_formula( who );
