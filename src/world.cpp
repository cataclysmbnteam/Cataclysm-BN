...
bool WORLDINFO::save( const bool is_conversion ) const
{
    ...
    if( world_save_format == save_format::V2_COMPRESSED_SQLITE3 && !file_exist( folder_path() + "/map.sqlite3" ) ) {
...
static bool read_from_db( sqlite3 *db, const std::string &path, file_read_fn reader, bool optional ) {
...
static bool read_from_db_json( sqlite3 *db, const std::string &path, file_read_json_fn reader, bool optional )
{
