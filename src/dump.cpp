                       ...
                       const std::vector<std::string> &opts )
{
    ...
    standard_npc const test_npc( "S1", { 0, 0, 2 }, { "gloves_survivor", "mask_lsurvivor" },
    ...
    if( what == "AMMO" ) {
        ...
        for( const itype *e : item_controller->all() ) {
            item  const &food = *item::spawn_temporary( e, calendar::turn, item::solitary_tag {} );
        ...
        auto dump = [&rows]( const vpart_info & obj ) {
            ...
            int const w = std::ceil( to_gram( item::spawn_temporary( obj.item )->weight() ) / 1000.0 );
