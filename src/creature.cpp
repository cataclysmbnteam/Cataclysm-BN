                                       ...
                                       dealt_projectile_attack &attack )
{
    ...
    double const hit_value = std::abs( goodhit + rng_float( -hit_location_variance,
    ...
    if( proj.has_effect( ammo_effect_NOGIB ) ) {
        float const dmg_ratio = impact.total_damage() / get_hp_max( bp_hit );
        if( dmg_ratio > 1.25f ) {
    ...
    float const dmg_before_armor = std::max( 1.0f, impact.total_damage() );
    ...
    float const dmg_after_armor = dealt_dam.total_damage();
    ...
    for( auto &elem : proj.impact.damage_units ) {
        ...
        float const damage_ratio = dmg_after_armor / dmg_before_armor;
...
{
    int const speed = std::round( ( get_speed_base() + get_speed_bonus() ) * ( 1 + get_speed_mult() ) );
...
void Creature::draw( const catacurses::window &w, const tripoint &origin, bool inverted ) const
{
    ...
    point const draw( -origin.xy() + point( ( getmaxx( w ) / 2 ) + posx(),
                                            ( getmaxy( w ) / 2 ) + posy() ) );
    if( inverted ) {
