...
const weather_type_id &weather_generator::get_weather_conditions( const w_point &w ) const
{
    ...
    for( const weather_type_id &type : weather_types ) {
        const weather_requirements &requirement = type->requirements;
        weather_requirements rq2 = requirement;
        bool test_pressure =
            requirement.pressure_max > w.pressure &&
            requirement.pressure_min < w.pressure;
        bool test_humidity =
            requirement.humidity_max > w.humidity &&
            requirement.humidity_min < w.humidity;
        if( ( requirement.humidity_and_pressure && !( test_pressure && test_humidity ) ) ||
            ( !requirement.humidity_and_pressure && !( test_pressure || test_humidity ) ) ) {
            continue;
        }
        bool test_temperature =
            requirement.temperature_max > units::to_fahrenheit( w.temperature ) &&
            requirement.temperature_min < units::to_fahrenheit( w.temperature );
        bool test_windspeed =
            requirement.windpower_max > w.windpower &&
            requirement.windpower_min < w.windpower;
        bool test_acidic = !requirement.acidic || w.acidic;
        if( !( test_temperature && test_windspeed && test_acidic ) ) {
            continue;
        }

        if( !requirement.required_weathers.empty() ) {
            if( std::find( requirement.required_weathers.begin(), requirement.required_weathers.end(),
                           *current_conditions ) == requirement.required_weathers.end() ) {
                continue;
            }
        }

        if( requirement.time != weather_time_requirement_type::both ) {
            bool day = is_day( calendar::turn );
            if( ( requirement.time == weather_time_requirement_type::day && !day ) ||
                ( requirement.time == weather_time_requirement_type::night && day ) ) {
                continue;
            }
        }
