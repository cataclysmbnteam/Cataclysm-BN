...
bool is_number_integer( sol::state_view lua, const sol::object &val )
{
    ...
    if( res.status() != sol::call_status::ok ) {
        sol::error const err = res;
        std::string const msg = string_format( "is_number_integer runtime error: %s", err.what() );
...
std::optional<std::string> get_luna_type( const sol::object &val )
{
    ...
    if( val.get_type() == sol::type::userdata ) {
        sol::protected_function const glt = lua.script( "return function(a) return a:get_luna_type() end" );
        if( glt ) {
            sol::protected_function_result const res = glt( val );
            if( res.status() == sol::call_status::ok ) {
...
bool compare_values( const sol::object &a, const sol::object &b )
{
    ...
    sol::type const type = a.get_type();
    if( type != b.get_type() ) {
        return false;
    }
    ...
    if( type == sol::type::userdata ) {
        std::optional<std::string> const luna_a = get_luna_type( a );
        std::optional<std::string> const luna_b = get_luna_type( b );
        if( luna_a != luna_b ) {
            return false;
        }
    }
    ...
    sol::protected_function const cmp_func = lua.script( "return function(a, b) return a == b end" );
    sol::protected_function_result const res = cmp_func( a, b );
    if( res.status() != sol::call_status::ok ) {
        sol::error const err = res;
...
bool compare_tables( sol::table a, sol::table b )
{
    ...
    a.for_each( [&]( sol::object a_key, const sol::object & a_val ) {
        ...
        sol::object const b_key = find_equivalent_key( b, std::move( a_key ) );
        if( b_key == sol::nil ) {
            are_equal = false;
            return;
        } else {
            sol::object const b_val = b[b_key];
            if( b_val == sol::nil ) {
                are_equal = false;
    ...
    if( are_equal ) {
        b.for_each( [&]( sol::object b_key, const sol::object & /*b_val*/ ) {
            sol::object const a_key = find_equivalent_key( a, std::move( b_key ) );
            if( a_key == sol::nil ) {
                are_equal = false;
            } else {
                sol::object const a_val = a[a_key];
                if( a_val == sol::nil ) {
                    are_equal = false;
                }
            }
