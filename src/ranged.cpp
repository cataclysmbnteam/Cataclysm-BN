    ...
    if( shooter.is_underwater() && !parent.has_flag( flag_WATERPROOF_GUN ) &&
        ...
        for( auto mod : parent.gunmods() ) {
            if( mod->has_flag( flag_CONSUMABLE ) ) {
                ...
                int const initstate = parent.damage();
                      ...
                      item *ammo )
{
    ...
    while( curshot != shots ) {
        ...
        if( gun.gun_skill() == skill_throw && !who.is_fake() && gun.ammo_data() ) {
            item  const &tmp = *item::spawn_temporary( item( gun.ammo_data() ) );
        ...
        int const noise = calc_gun_volume( gun );
...
float get_str_draw_penalty( const item &it, const Character &p )
{
    ...
    float const archer_str = std::max( 1, p.get_str() + str_adjust );
...
int ranged::time_to_attack( const Character &p, const item &firing, const item *loc )
{
    ...
    if( !loc ) {
        ...
        item_reload_option const opt = item_reload_option( p.as_player(), const_cast<item *>( &firing ),
                                       &firing,
                                       *const_cast<item *>( loc ) );
...
item::sound_data item::gun_noise( const bool burst ) const
{
    ...
    int const noise = calc_gun_volume( *this );
...
std::vector<weak_ptr_fast<Creature>> target_ui::list_friendlies_in_lof()
{
    ...
    for( const tripoint &p : traj ) {
        if( p != dst && p != src ) {
            ...
            if( cr && you->sees( *cr ) ) {
                Attitude const a = cr->attitude_to( *this->you );
                if(
...
{
    item_reload_option const opt = character_funcs::select_ammo( u, w, false, true, true );
    if( opt ) {
...
double ranged::aim_multiplier_from_volume( const item &gun )
{
    ...
    double const aim_mult = ( 10000.0 - static_cast<float>( gun.volume() / 1_ml ) ) / 10000.0;
