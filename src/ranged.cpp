                                  ...
                                  bool grey_out_firing_controls = false )
{
    ...
    if( mode == TARGET_MODE_TURRET ) {
        mvwprintz( w_target, point( 1, text_y++ ), c_white,
                   _( "[%c] to show/hide approx.  lines of fire." ),
...
std::vector<tripoint> target_handler_old::target_ui( player &pc, target_mode mode,
...
// magic mod
std::vector<tripoint> target_handler_old::target_ui( spell_id sp, const bool no_fail,
        const bool no_mana )
{
...
std::vector<tripoint> target_handler_old::target_ui( spell &casting, const bool no_fail,
        const bool no_mana )
{
    player &pc = g->u;
