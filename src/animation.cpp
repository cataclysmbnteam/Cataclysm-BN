                            ...
                            const nc_color &col )
{
    ...
    int frame = 0;
    const shared_ptr_fast<game::draw_callback_t> explosion_cb =
...
void draw_bullet_curses( map &m, const tripoint &t, const char bullet, const tripoint *const p )
{
    ...
    const shared_ptr_fast<game::draw_callback_t> bullet_cb =
    make_shared_fast<game::draw_callback_t>( [&]() {
                        ...
                        const std::vector<tripoint> &trajectory, const char bullet )
{
    ...
    const shared_ptr_fast<draw_callback_t> bullet_cb = make_shared_fast<draw_callback_t>( [&]() {
                    ...
                    const std::string &cTile )
{
    ...
    if( init_pos.z == 0 && is_valid_in_w_terrain( init_pos.xy() ) ) {
        const shared_ptr_fast<game::draw_callback_t> hit_cb =
        make_shared_fast<game::draw_callback_t>( [&]() {
...
void game::draw_hit_mon( const tripoint &p, const monster &m, const bool dead )
{
    ...
    const shared_ptr_fast<draw_callback_t> hit_cb = make_shared_fast<draw_callback_t>( [&]() {
...
static void draw_cone_aoe_curses( const tripoint &, const bucketed_points &waves )
{
    ...
    auto it = waves.begin();
    const shared_ptr_fast<game::draw_callback_t> wave_cb =
    make_shared_fast<game::draw_callback_t>( [&]() {
