...
{
    std::string i_name_w;
    item  const &weapon = primary_weapon();
    if( !weapon.is_null() ) {
...
std::map<bodypart_id, float> Character::bodypart_exposure()
{
    ...
    for( const item * const &it : worn ) {
        ...
        for( const bodypart_id &bp : all_body_parts )  {
            ...
            float const part_exposure = 1.0 - ( it->get_coverage( bp ) / 100.0f );
...
void Character::suffer_from_other_mutations()
{
    ...
    if( has_trait( trait_SORES ) ) {
        for( const body_part bp : all_body_parts ) {
            ...
            int const sores_pain = 5 + ( 0.4 * std::abs( encumb( convert_bp( bp ) ) ) );
...
void Character::apply_wetness_morale( const units::temperature &temperature )
{
    ...
    for( std::pair<const bodypart_str_id, bodypart>  const &elem : get_body() ) {
        ...
        int const temp_cur = iter->second.get_temp_cur();
