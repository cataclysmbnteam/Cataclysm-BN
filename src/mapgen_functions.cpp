...
void mapgen_sewer( mapgendata &dat )
{
    ...
    bool sewer_nesw[4] = {};
    int const num_dirs = terrain_type_to_nesw_array( dat.terrain_type(), sewer_nesw );
...
void mapgen_lake_shore( mapgendata &dat )
{
    ...
    for( int i = 0; i < 8; i++ ) {
        ...
        int const next = i % 2 == 0 ? 1 : -1;
        int const dir = slots.contains( i + next ) ? -next : next;
        ...
        if( pair == i ) {
            ...
            bool const water[8] = { n_water, n_water, w_water, w_water, s_water, s_water, e_water, e_water };
            int fallback = i;
            for( int j = modulo( i + dir, 8 ); j != i; j = modulo( j + dir, 8 ) ) {
                if( !water[j] ) {
                    fallback = j;
    ...
    ter_id const edge_tile = dat.zlevel() >= 0 ? t_water_sh : t_rock;
