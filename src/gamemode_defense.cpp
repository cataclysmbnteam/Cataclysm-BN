    ...
    while( !done ) {
        ...
        if( action == "HELP" ) {
            ...
            if( current_window == 1 && !items[category_selected].empty() ) {
                ...
                int const item_price = item::spawn_temporary( tmp_itm,
                         ...
                         std::vector<int> *counts, int offset, int item_selected )
{
    ...
    if( item_selected < static_cast<int>( items->size() ) ) {
        item  const &tmp = *item::spawn_temporary( ( *items )[item_selected],
    ...
    for( int i = offset; i <= offset + FULL_SCREEN_HEIGHT - 2 &&
        ...
        if( ( *counts )[i] > 0 ) {
            int const item_price = item::spawn_temporary( ( *items )[i],
                                   calendar::start_of_cataclysm )->price( false );
