...
bool mattack::acid_barf( monster *z )
{
    ...
    bodypart_str_id const hit = target->get_random_body_part().id();
    int dam = rng( 5, 12 );
    ...
    if( !target->uncanny_dodge() ) {
        ...
        if( rng( 0, 10 ) > target->get_dodge() || one_in( target->get_dodge() ) ) {
            ...
            for( int i = 0; i < rng( 2, 4 ); i++ ) {
                bodypart_str_id const bp = random_body_part();
...
bool mattack::fungus_advanced( monster *z )
{
    ...
    double const creatures_threshold = static_cast<float>( fungal_opt.advanced_creatures_threshold );
    int const creatures_nearby = static_cast<int>( g->num_creatures() );
...
void mattack::taze( monster *z, Creature *target )
{
    ...
    int const dam = target->deal_damage( z, target->get_random_body_part(),
...
bool mattack::command_buff( monster *z )
{
    ...
    for( monster &ally : g->all_monsters() ) {
        if( rl_dist_fast( ally.pos(), z->pos() ) <= 30 &&
            ...
            time_duration const half_max_dur = effect_command_buff->get_max_duration() / 2;
...
bool mattack::kamikaze( monster *z )
{
    ...
    if( exp_actor->explosion ) {
        int const tmp = ( exp_actor->explosion.safe_range() / 2 );
