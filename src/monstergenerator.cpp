...
void mtype::load( const JsonObject &jo, const std::string &src )
{
    ...
    if( jo.has_object( "on_death" ) ) {
        JsonObject const od = jo.get_object( "on_death" );
        ...
        if( od.has_object( "spawn_mon_near" ) ) {
            JsonObject const od_smn = od.get_object( "spawn_mon_near" );
            int dist;
    ...
    std::unordered_set<std::string> const blacklisted_specials {"PARROT", "PARROT_AT_DANGER", "EAT_CROP", "EAT_FOOD"};
