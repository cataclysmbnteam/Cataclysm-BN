...
std::map<item *, bionic_id> npc::check_toggle_cbm()
{
    ...
    for( bionic  const &bio : get_bionic_collection() ) {
...
void npc::check_or_use_weapon_cbm()
{
    ...
    if( !avail_active_cbms.empty() ) {
        ...
        for( int const i : avail_active_cbms ) {
            ...
            double const dps = cbm_weapon->ideal_ranged_dps( *this, mode_ );
...
{
    int const amount_kj = units::to_kilojoule( amount );
    ...
    if( bio.id == bio_remote ) {
        ...
        if( calendar::once_every( 30_turns ) ) {
            ...
            if( calendar::once_every( 2_minutes ) ) {
                ...
                for( bodypart_id  const &bp : damaged_parts() ) {
                    if( !can_use_bionic() ) {
                        return;
                    }
