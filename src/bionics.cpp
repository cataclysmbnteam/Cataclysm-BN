    ...
    if( !avail_active_cbms.empty() ) {
        ...
        for( int i : avail_active_cbms ) {
            ...
            if( wield_gun && active_index < 0 ) {
                gun_mode weap_mode = npc_ai::best_mode_for_range( *this, this->weapon, dist ).second;
                dps = this->weapon.ideal_ranged_dps( *this, weap_mode );
            }
...
bool Character::deactivate_bionic( bionic &bio, bool eff_only )
{
    ...
    if( bio.info().has_flag( flag_BIONIC_WEAPON ) ) {
        if( weapon.typeId() == bio.info().fake_item ) {
            ...
            if( g->u.sees( pos() ) ) {
                add_msg_if_npc( m_info, _( "<npcname> withdraws their %s." ), weapon.tname() );
            }
            bio.ammo_loaded =
