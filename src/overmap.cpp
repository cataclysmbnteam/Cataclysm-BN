...
class joins_tracker
{
    public:
        ...
        void add_joins_for(
            ...
            std::unordered_set<om_pos_dir> const avoid(
...
struct mutable_overmap_phase_remainder {
    ...
    std::optional<can_place_result> can_place(
        ...
        std::vector<mutable_overmap_piece_candidate> const pieces = rule.pieces( origin, dir );
...
void overmap_special::check() const
{
    ...
    std::unordered_set<overmap_special_id> const parents{ id };
    ...
    const city &cit, const bool must_be_unexplored, const bool force )
{
    ...
    for( const auto &nested : special.get_nested_specials() ) {
        ...
        if( can_place_special( *nested.second, rp, dir, false ) ) {
            std::vector<tripoint_om_omt> const nested_result = place_special( *nested.second, rp, dir,
                                    ...
                                    specials_overlay &points, const bool must_be_unexplored )
{
    ...
    for( auto p = points.begin(); p != points.end(); ) {
        ...
        std::vector<tripoint_om_omt> const result = place_special( special, *p, rotation, nearest_city,
                false,
                must_be_unexplored );
        if( need_city ) {
