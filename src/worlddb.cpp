    ...
    ,worlddir( path )
{
    ...
    if (SQLITE_OK != (ret = sqlite3_open_v2(db_path.c_str(), &db, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, NULL))) {
...
bool worlddb::file_exist_in_db( const std::string &path ) {
    ...
    if (sqlite3_bind_text(stmt, sqlite3_bind_parameter_index(stmt, ":path"), path.c_str(), -1, SQLITE_TRANSIENT) != SQLITE_OK) {
...
void worlddb::write_to_file( const std::string &path, const std::function<void( std::ostream & )> &writer )
{
    ...
    if (sqlite3_bind_text(stmt, sqlite3_bind_parameter_index(stmt, ":path"), path.c_str(), -1, SQLITE_TRANSIENT) != SQLITE_OK ||
        sqlite3_bind_text(stmt, sqlite3_bind_parameter_index(stmt, ":parent"), parent.c_str(), -1, SQLITE_TRANSIENT) != SQLITE_OK ||
        sqlite3_bind_text(stmt, sqlite3_bind_parameter_index(stmt, ":data"), data.c_str(), -1, SQLITE_TRANSIENT) != SQLITE_OK) {
...
bool worlddb::write_to_file( const std::string &path, const std::function<void( std::ostream & )> &writer,
...
bool worlddb::read_from_file( const std::string &path, const std::function<void( std::istream & )> &reader )
{
