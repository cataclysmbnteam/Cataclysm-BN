...
bool leap_actor::call( monster &z ) const
{
    ...
    const tripoint target = z.move_target();
    const float best_float = trigdist ? trig_dist( z.pos(), target ) : square_dist( z.pos(), target );
    ...
    for( const tripoint &candidate : here.points_in_radius( z.pos(), max_range ) ) {
        ...
        const float leap_dist = trigdist ? trig_dist( z.pos(), candidate ) :
                                square_dist( z.pos(), candidate );
        if( leap_dist > max_range || leap_dist < min_range ) {
            continue;
        }
        const int candidate_dist = rl_dist( candidate, target );
    ...
    for( const auto &candidate : candidates ) {
        ...
        const std::vector<tripoint> line = here.find_clear_path( z.pos(), dest );
...
void melee_actor::load_internal( const JsonObject &obj, const std::string & )
{
    ...
    if( obj.has_array( "body_parts" ) ) {
        for( const JsonArray sub : obj.get_array( "body_parts" ) ) {
    ...
    if( obj.has_array( "effects" ) ) {
        for( const JsonObject eff : obj.get_array( "effects" ) ) {
...
bool melee_actor::call( monster &z ) const
{
    ...
    const int hitspread = target->deal_melee_attack( &z, dice( acc, 10 ) );
    ...
    const double multiplier = rng_float( min_mul, max_mul );
    ...
    const int damage_total = dealt_damage.total_damage();
...
void gun_actor::load_internal( const JsonObject &obj, const std::string & )
{
    ...
    if( obj.has_array( "fake_skills" ) ) {
        for( const JsonArray cur : obj.get_array( "fake_skills" ) ) {
    ...
    for( const JsonArray mode : obj.get_array( "ranges" ) ) {
...
auto find_target_vehicle( monster &z, int range ) -> std::optional<tripoint>
{
    ...
    for( const wrapped_vehicle &v : here.get_vehicles() ) {
        ...
        for( const vpart_reference &vp : v.v->get_avail_parts( "CONTROLS" ) ) {
            ...
            const int new_dist = rl_dist( z.pos(), vp.pos() );
            if( new_dist <= range ) {
                aim_at = vp.pos();
                range = new_dist;
                found = true;
        ...
        if( !found_controls ) {
            const std::vector<tripoint> line = here.find_clear_path( z.pos(), v.v->global_pos3() );
            tripoint prev_point = z.pos();
            for( const tripoint &i : line ) {
                if( !z.sees( i ) ||  here.floor_between( prev_point, i ) ) {
                    break;
                }
                optional_vpart_position vp = here.veh_at( i );
                if( vp && &vp->vehicle() == v.v ) {
                    const int new_dist = rl_dist( z.pos(), i );
                    if( new_dist <= range ) {
                        aim_at = i;
                        range = new_dist;
                        found = true;
                       ...
                       int inital_recoil ) const
{
    ...
    const itype_id ammo = ammo_type ? ammo_type : gun->ammo_default();
    if( ammo ) {
    ...
    const int qty = gun->gun_current_mode().qty;
