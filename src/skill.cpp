...
skill_id Skill::from_legacy_int( const int legacy_id )
{
    static const std::array<skill_id, 28> legacy_skills = { {
            ...
            skill_id( "firstaid" ), skill_id( "speech" ), skill_id( "speech" ), skill_id( "computer" ),
...
// Actually take the difference in speech skill between the two parties involved
// Caps at 200% when you are 5 levels ahead, int comparison is handled in npctalk.cpp
double price_adjustment( int speech_skill )
{
    if( speech_skill <= 0 ) {
        return 1.0;
    }
