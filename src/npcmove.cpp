...
void npc::execute_action( npc_action action )
{
    ...
    */
    ...
    switch( action ) {
        ...
        case npc_aim: {
            gun_mode const mode = cbm_active.is_null() ? primary_weapon().gun_current_mode() :
...
auto item::ideal_ranged_dps( const Character &who, std::optional<gun_mode> &mode ) const -> double
{
    ...
    float const damage_factor = gun_damage.total_damage() * burst_size;
...
{
    int const rltime = item_reload_cost( gun, *item::spawn_temporary( gun.ammo_default() ),
                                         gun.ammo_capacity() );
...
void npc::move_to( const tripoint &pt, bool no_bashing, std::set<tripoint> *nomove )
{
    tripoint p = pt;
    map &here = get_map();
    bool const ceiling_blocking_climb = !here.has_floor_or_support( pos() ) ||
...
void npc::mug_player( Character &mark )
{
    ...
    const_invslice const slice = mark.inv_const_slice();
...
bool npc::complain()
{
    ...
    if( has_effect( effect_infected ) ) {
        bodypart_str_id const bp = bp_affected( *this, effect_infected );
    ...
    if( has_effect( effect_bite ) ) {
        bodypart_str_id const bp = bp_affected( *this, effect_bite );
    ...
    if( has_effect( effect_bleed ) ) {
        bodypart_str_id const bp = bp_affected( *this, effect_bleed );
...
{
    item_reload_option const reload_opt = character_funcs::select_ammo( *this, it );
