...
{
    {
        read_lock<std::shared_mutex> const _l( mutex );
    ...
    overmap *new_om;
    {
        write_lock<std::shared_mutex> const _l( mutex );
        ...
        const omt_find_params &params )
{
    ...
    if( params.om_special ) {
        bool const meets_om_special = om_loc.om->check_overmap_special_type( *params.om_special,
                                      om_loc.local );
        if( !meets_om_special ) {
            return false;
        }
...
struct find_task_generator {
    ...
    auto get_om_loc() {
        auto &p = *_it;
        tripoint_abs_omt const loc( p.x, p.y, 0 );
        point_abs_om om_pos;
        point_om_omt local;
    ...
    std::optional<find_task> operator()() {
        ...
        point_abs_om const om_loc = _current.first;

        for( int n = 0; n < _n_steps; n++ ) {
            auto &p = *_it;
            for( int z = _min_z; z <= _max_z; z++ ) {
                tripoint_abs_omt const abs( p.x, p.y, z );
                tripoint_om_omt const om( _current.second, z );
        ...
        const omt_find_params &params )
{
    ...
    while( true ) {
        ...
        for( const auto &loc : task_omts ) {
            overmap_with_local_coords const q{ om_loc, loc.second };
        ...
        const omt_find_params &pp )
{
    ...
    for( auto &p : scan_result ) {
        int const dist = square_dist( origin, p );
        if( dist < min_dist ) {
            near_points.clear();
            min_dist = dist;
