...
void game::suggest_auto_walk_to_stairs( Character &u, map &m, const std::string &direction )
{
    ...
    bool const is_elevator = m.has_flag( TFLAG_ELEVATOR, *stair_pos );
...
void game::process_voluntary_act_interrupt()
{
    ...
    bool const has_activity = u.activity && !u.activity->complete();
...
void game::start_hauling( const tripoint &pos )
{
    ...
    map_stack const items = m.i_at( pos );
...
void game::process_artifact( item &it, Character &who )
{
    ...
    for( const art_effect_passive &i : effects ) {
        switch( i ) {
            ...
            case AEP_SMOKE:
                if( one_in( 10 ) ) {
                    tripoint const pt( who.posx() + rng( -1, 1 ),
                                       who.posy() + rng( -1, 1 ),
                                       who.posz() );
