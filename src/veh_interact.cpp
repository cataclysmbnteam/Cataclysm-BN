...
void veh_interact::display_stats() const
{
    ...
    int i = 0;
    // this lambda simplifies printing stat for vehicle line-by-line
    ...
    if( is_aircraft ) {
        print_stat( w_stats, point( x[i], y[i] ), w[i], c_light_gray,
    ...
    print_stat( w_stats, point( x[i], y[i] ), w[i], total_durability_color, total_durability_text );
    ...
    print_stat_if( is_boat, w_stats, point( x[i], y[i] ), w[i], c_light_gray,
                   _( "Water drag:     <color_light_blue>%5.2f</color>" ),
                   veh->coeff_water_drag() );

    print_stat_if( is_ground, w_stats, point( x[i], y[i] ), w[i], c_light_gray,
