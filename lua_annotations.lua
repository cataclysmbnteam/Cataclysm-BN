...
---@class hooks
---@field on_character_death fun() @Called when a character is dead.<br />The hook receives a table with keys: `char` and `killer`.
---@field on_character_reset_stats fun() @Called when character stat gets reset.<br />The hook receives a table with a key: `character`.
---@field on_creature_blocked fun() @Called when a character successfully blocks.<br />The hook receives a table with keys: `char`, `source`, `bodypart_id`, `damage_instance`, and `damage_blocked` (float).
---@field on_creature_dodged fun() @Called when a character or monster successfully dodges.<br />The hook receives a table with keys: `char` (or `mon`), `source`, and `difficulty`.
---@field on_creature_melee_attacked fun() @Called after a character or monster has attacked in melee.<br />The hook receives a table with keys: `char` (or `mon`), `target`, and `success` (boolean).
---@field on_creature_performed_technique fun() @Called when a character has performed a technique.<br />The hook receives a table with keys: `char`, `technique`, `target`, `damage_instance`, and `move_cost` (final cost).
...
---@field on_mon_death fun() @Called when a monster is dead.<br />The hook receives a table with keys: `mon` and `killer`.
hooks = {}
