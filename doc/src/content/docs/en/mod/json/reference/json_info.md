...
- `cast_spell` - Casts the spell specified by the trap's `spell_data`, centered on whatever set it off. Note that the spell used generally requires a `min_aoe` defined to work successfully, and not all spell effects can be expected to work properly with this.
